...
def _report(execution_id: str, final: dict) -> None:
    telemetry.execution_end(execution_id, final.get("ticket_id", ""),
                            final.get("final_status", "failed"), final.get("route", "coding"))
    print(
        f"\n[DONE] status={final.get('final_status')} "
        f"result={final.get('automation_result')} "
        f"failure_class={final.get('failure_class') or '-'} "
        f"ready_flipped={final.get('ready_flipped', False)} "
        f"pr=#{final.get('pr_number')} test_automation_pr={final.get('test_automation_pr_url') or '-'}"
    )
    print(f"       outcome: {final.get('final_outcome')}")


async def _drive_stream(app, initial, thread) -> dict:
    """Stream node completions for live progress, then return the final state."""
...
async def _execute(execution_id: str, ticket_id: str, initial, thread) -> None:
    ...
    try:
        async with AsyncSqliteSaver.from_conn_string(config.CHECKPOINT_DB) as saver:
            ...
            final = await _drive_stream(app, initial, thread)
        _report(execution_id, final)
...
async def _run(ticket_id: str, context: str) -> None:
    ...
    print(f"[ocean-pipeline] {ticket_id}  execution={execution_id}")
